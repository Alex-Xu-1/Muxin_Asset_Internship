...
class Strategy:
    ...
    def on_func(self,
        ...
        self.df_filtered = self.df_merged[mask]
        self.df_filtered = self.df_filtered[self.df_filtered['if_listing'] == 1]
        
        self.df_filtered = self.df_filtered[self.df_filtered['if_delist_period'] == 0]
        self.df_filtered = self.df_filtered[self.df_filtered['close'] >= 1]
        self.df_filtered = self.df_filtered[self.df_filtered['if_ST'] == 0]

        # Filter out the 400 minimum market value stocks in each day
        self.df_min_400 = self.df_filtered.groupby(level=0).apply(lambda x: x.nsmallest(400, 'mkt_val'))

        self.df_min_400 = self.df_min_400.reset_index(level=0, drop=True)

        self.df_min_400 = self.df_min_400.reset_index()

        df_sec = pd.DataFrame(weights)

        selected_stocks = self.df_min_400[self.df_min_400['date'] == date]['qscode']
